...
const handleAddkey= async (e) => {
  e.preventDefault();
    try{
      await axios.post('/api/LicensSettings/',license)
  ...
  return (
    ...
    <div sx={{ display: 'flex', flexDirection:'raw' ,justifyContent: 'center'}} >
      <Box sx={{ display: 'flex',flexDirection:'column' , justifyContent: 'center', alignItems:"center", padding:"80px", marginTop:"50px" }}>
          ...
          onChange={handleFile} 
          color="secondary"
          onClick={(e) => e.target.value = null} // Permet de réinitialiser la sélection lors de l'ouverture
          inputProps={{
            accept: '.xml', // Filter to show only XML files
          }}
          />
        <br/>
