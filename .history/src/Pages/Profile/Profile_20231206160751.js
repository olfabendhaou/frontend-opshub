...
const Profile = () => { 
  ...
  const languages = ['Arabe', 'Anglais','Français'];
   ...
   const getTimeZone = async (countryCode) => {
    try {
      const response = await fetch(`https://api.timezonedb.com/v2.1/get-time-zone?key=YOUR_API_KEY&country=${countryCode}&format=json`);
      const data = await response.json();
      return data.zoneName || 'UTC'; // Use UTC as a fallback if no zoneName is available
    } catch (error) {
      console.error('Error fetching time zone:', error);
      return 'UTC'; // Use UTC as a fallback in case of an error
    }
  };
  ...
  const fetchProfileData = async () => {
            try {
              ...
              setSelectedCountry(usertoget.country)
          ...
          fetchProfileData();
          setSelectedTimeZone(getTimeZone(selectedcountry))
          .then((timeZone) => setSelectedTimeZone(timeZone))
...
<div className='Div'>
<label style={{display:'flex', justifyContent:'flex-start'}} >Country: </label>
<Select
style={{width: '90%', height:'33px',display:'flex', justifyContent:'flex-end'}}
name="country"
...
<div className='Div'>
<label style={{display:'flex', justifyContent:'flex-start'}}>Language:</label>
<Select
style={{width: '90%', height:'33px',display:'flex', justifyContent:'flex-end'}}
name="language"
...
style={{width: '90%', height:'33px',display:'flex', justifyContent:'flex-end'}}
name="timezone"
value={getTimeZones(selectedTimeZone)}
onChange={handleInputChange}
>
{timeZones.map((zone) => (
