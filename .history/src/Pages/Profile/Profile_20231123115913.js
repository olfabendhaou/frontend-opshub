...
const Profile = ({ setAuth }) => { 
  ...
  const [errorMessage, setErrorMessage] = useState(nulle);
  ...
  const fetchProfileData = async () => {
          ...
          const config = {
            ...
            if (error.response) {
              ...
              setErrorMessage(prevErrors => [...prevErrors, error.response.data.errors[0].msg]);
  ...
  {Array.isArray(errorMessage)&& errorMessage.map((el)=>
     <Alert severity="warning">{el}</Alert>
