...
export default function Licenses() {
  ...
  const handleButtonClick = (e) => {
    e.preventDefault();
    if (!file) {
        showToastMessage("No selected file selected, Please Choose your file!","warning");
        return;
      }
    if (!loading) {
      setSuccess(false);
      ...
      <ToastContainer />
       ...
       {isUploading && 
        ...
        <form className='Form'>
            ...
            <Button variant="contained" width ="20px" color="primary" onClick={handleAddkey &&  navigate(-1)} >Save</Button>
            <ToastContainer />
            </form>
            </Box>
            </div>
