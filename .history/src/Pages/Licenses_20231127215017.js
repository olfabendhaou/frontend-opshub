...
export default function Licenses() {
  ...
  const [licensekey,setLicense_key] = useState(null);
  ...
  async function handleUpload(e) {
    try {
      ...
      showToastMessageerror("Upload Failed");
      console.error(error);
    }
  }
  ...
  const handleAddkey= async (e) => {
    ...
    try{
      const response = await axios.post('http://localhost:8000/api/LicensSettings/',licensekey)
      console.log(response.data);
