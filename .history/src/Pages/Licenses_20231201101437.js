...
///////////////Input file functions////////
  ...
  async function handleUpload(e) {
    try {
      ...
      if (match) {
        ...
        setLicense_key(extractedLicenseKey);
        setisUploading(true)
    } else {
 ...
 const handleKey =(e ) =>{
  setLicense_key({...license_key,[e.target.name]: e.target.value});
};
  ...
  const handleAddkey= async (e) => {
    e.preventDefault();
    try{
      const response = await axios.post('http://localhost:8000/api/LicensSettings/',license)
      console.log(response.data);
