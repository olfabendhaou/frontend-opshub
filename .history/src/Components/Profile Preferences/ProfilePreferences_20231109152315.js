...
const ProfilePreferences = ({setAuth}) => {
  const match =useMatch("http://localhost:8000/api/user/password-reset/:id")
  ...
  const handleUpdatepassword = async () => {
    try {
      ...
      await axios.put(match.params.id, user,config);
      setAuth(true)
      // setIsEditing(false);
  ...
  return (
    ...
    marginTop:"200px" }}>
      ...
      <FormControl sx={{ m: 1, width: '25ch' }} variant="standard">
        ...
        <Input
          onChange={handleChange} name='confirmnewassword'
          id="standard-adornment-password"
          type={showPassword ? 'text' : 'password'}
          endAdornment={
            <InputAdornment position="end">
