...
const fetchProfileData = async () => {
  ...
  const handleChange =(e)=>{
    setPassword({...passsword, [e.target.name] : e.target.value});
  };
  const handleUpdatepassword = async () => {
    try {
      const config = {
        ...
        const requestBody = {
       password: JSON.stringify(confi
        };
      await axios.put(`http://localhost:8000/api/user/password-reset/${parseInt(user.id,10)}`,,config);
      setAuth(true)
      // setIsEditing(false);
    } catch (error) {
