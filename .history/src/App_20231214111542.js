...
const checkTokenExpiration=async (token) =>  {
  try{
   ...
   if (timeUntilExpiration > 0) {
        // Schedule the action to be performed when the token expires
        const logoutTimeout = setTimeout(() => {
          // Perform the logout action
          logout();
        }, timeUntilExpiration);

        return () => clearTimeout(logoutTimeout); // Cleanup the timeout on component unmount
      } else {
        // The token is already expired
        console.log('Token is already expired');
        // Optionally, perform the logout action immediately
        // logout();
      }
    };
