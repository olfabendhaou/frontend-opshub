...
useEffect(() => {
    // Function to handle the logic when the token is present
    ...
    // Function to handle the logic when the token is not present
    const handleTokenNotPresent = () => {
        logoutAction(user);
    };
    ...
    if (token) {
        // If the token is present, remove it, and then handle the logic
        localStorage.removeItem("token");
        handleTokenPresent();
    } else {
        // If the token is not present, directly handle the logic
        handleTokenNotPresent();
    }

    // Cleanup step: Remove the token when the component is unmounted
    return () => {
        localStorage.removeItem("token");
    };

}, [user, setIsAuthenticated, fetchProfileData]);
